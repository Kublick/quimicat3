...
import { IMuestra, muestrasValidation } from '../../../intefaces';
...
export const MuestraModal: FC<Props> = ({
	...
	const createMuestra = trpc.useMutation(['configuracion.createMuestra'], {
		onSuccess: () => {
			//invalidate cache
			utils.invalidateQueries('configuracion.getMuestras');
		},
	});

	const updateMuestra = trpc.useMutation(['configuracion.updateMuestra'], {
		onSuccess: () => {
			//invalidate cache
			utils.invalidateQueries('configuracion.getMuestras');
		},
	...
	const onSubmit = (data: IMuestra) => {
		...
		if (mode === 'new') {
			...
			toast.success('Muestra creado correctamente');
		}

		if (mode === 'edit') {
			updateMuestra.mutateAsync(data);
			setMuestra(null);
			toast.success('Muestra actualizado correctamente');
		}
