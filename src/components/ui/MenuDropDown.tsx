...
const MenuDropDown: FC<Props> = ({ data }) => {
  ...
  return (
    <>
      {data.map(({ id, title, menuItems, feature, href }) => (
        <div key={id}>
          {href === "" ? (
            <>
              {isEnabled(feature) && (
                <Collapse.Group>
                  <Collapse
                    title={title}
                    arrowIcon={<ChevronUpIcon className="h-6 w-6 text-white" />}
                    css={{
                          ...
                          <button
                            className={`${
                              router.pathname === item.href
                                ...
                                : "bg-slate-200 "
                            } w-full cursor-pointer rounded-lg border-none p-2 hover:opacity-90`}
                          >
                            {item.label}
                          </button>
                        </Link>
